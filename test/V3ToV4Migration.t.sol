...
import {IHuskeyV3PoolDeployer} from "../src/modules/huskey/v3/interfaces/IHuskeyV3PoolDeployer.sol";
import {IHuskeyV3Factory} from "../src/modules/huskey/v3/interfaces/IHuskeyV3Factory.sol";
...
contract V3ToV4MigrationTest is
    BaseCatalistSwapV4,
    OldVersionHelper,
    BinLiquidityHelper
{
    ...
    function test_v3PositionManager_onlyAuthorizedForToken() public {
        ...
        IV3NonfungiblePositionManager.CollectParams
            memory params = IV3NonfungiblePositionManager.CollectParams({
                tokenId: v3TokenId,
        ...
        bytes memory commands = abi.encodePacked(
            bytes1(uint8(Commands.V3_POSITION_MANAGER_CALL))
        );
        bytes[] memory inputs = new bytes[](1);
        inputs[0] = abi.encodePacked(
            IV3NonfungiblePositionManager.collect.selector,
            abi.encode(params)
        );

        vm.expectRevert(
            abi.encodeWithSelector(
                V3ToV4Migrator.NotAuthorizedForToken.selector,
                params.tokenId
            )
        );
    ...
    function test_v3PositionManager_invalidAction() public {
        IV3NonfungiblePositionManager.MintParams
            memory params = IV3NonfungiblePositionManager.MintParams({
        ...
        bytes[] memory inputs = new bytes[](1);
        inputs[0] = abi.encodePacked(
            IV3NonfungiblePositionManager.mint.selector,
            abi.encode(params)
        );

        vm.expectRevert(
            abi.encodeWithSelector(
                V3ToV4Migrator.InvalidAction.selector,
                IV3NonfungiblePositionManager.mint.selector
            )
        );
        router.execute(commands, inputs);
    ...
    function test_v3PositionManager_erc721Permit() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) = _getErc721PermitSignature(
            address(router),
            v3TokenId,
            block.timestamp
        );
        ...
        bytes[] memory inputs = new bytes[](1);
        inputs[0] = abi.encodePacked(
            IERC721Permit.permit.selector,
            abi.encode(address(router), v3TokenId, block.timestamp, v, r, s)
        );
    ...
    function test_v3PositionManager_burn() public {
        ...
        IV3NonfungiblePositionManager.DecreaseLiquidityParams
            memory decreaseParams = IV3NonfungiblePositionManager
        ...
        IV3NonfungiblePositionManager.CollectParams
            memory collectParam = IV3NonfungiblePositionManager.CollectParams({
                tokenId: v3TokenId,
        ...
        inputs[3] = abi.encodePacked(
            IV3NonfungiblePositionManager.burn.selector,
    ...
    function test_v4CLPositionmanger_InvalidAction() public {
        ...
        bytes4 invalidSelector = IPositionManager
    ...
    function test_v4BinPositionmanager_BinAddLiquidity() public {
        ...
        vm.prank(alice);
        snapStart(
            "V3ToV4MigrationTest#test_v4BinPositionmanager_BinAddLiquidity"
        );
        router.execute(commands, inputs);
        snapEnd();
