...
/// @title Router for CatalistSwap Stable Trades
abstract contract StableSwapRouter is RouterImmutables, Permit2Payments, Ownable2Step {
  ...
  /**
   * @notice Set Catalist Stable Swap Factory and Info
  ...
  function _stableSwap(address[] calldata path, uint256[] calldata flag) private {
    unchecked {
      ...
      for (uint256 i; i < flag.length; i++) {
        ...
        (uint256 k, uint256 j, address swapContract) = stableSwapFactory.getStableInfo(
          input,
          output,
          flag[i]
        );
  ...
  /// @notice Performs a CatalistSwap stable exact input swap
  ...
  function stableSwapExactInput(
    ...
    if (
      amountIn != Constants.ALREADY_PAID && amountIn != ActionConstants.CONTRACT_BALANCE
    ) {
  ...
  /// @notice Performs a CatalistSwap stable exact output swap
  /// @param recipient The recipient of the output tokens
  ...
  function stableSwapExactOutputAmountIn(
    ...
    amountIn = stableSwapFactory.getStableAmountsIn(
      stableSwapInfo,
      path,
      flag,
      amountOut
