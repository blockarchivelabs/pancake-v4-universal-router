...
import {ICatalistV3Pool} from "../../src/modules/catalist/v3/interfaces/ICatalistV3Pool.sol";
import {V3SwapRouter} from "../../src/modules/catalist/v3/V3SwapRouter.sol";
import {Constants} from "../../src/libraries/Constants.sol";
import {Commands} from "../../src/libraries/Commands.sol";
import {RouterParameters} from "../../src/base/RouterImmutables.sol";

/// @dev fork test against BSC network at block 38560000
abstract contract CatalistSwapV3Test is Test, GasSnapshot {
    address constant RECIPIENT = address(10);
    uint256 constant AMOUNT = 1 ether;
    uint256 constant BALANCE = 100_000 ether;
    ICatalistV3Factory constant FACTORY =
        ICatalistV3Factory(0x0BFbCF9fa4f9C56B0F40a671Ad40E0805A091865);
    ICatalistV3PoolDeployer constant V3_DEPLOYER =
        ICatalistV3PoolDeployer(0x41ff9AA7e16B8B1a8a8dc4f0eFacd93D02d071c9);
    ...
    function test_v3Swap_ExactInput0For1() public {
        ...
        snapLastCall("CatalistSwapV3Test#test_v3Swap_ExactInput0For1");
    ...
    function test_v3Swap_ExactInput0For1_ContractBalance() public {
        ...
        router.execute(commands, inputs);
        snapLastCall(
            "CatalistSwapV3Test#test_v3Swap_ExactInput0For1_ContractBalance"
        );
    ...
    function test_v3Swap_exactInput_MultiHop() public {
        ...
        router.execute(commands, inputs);
        snapLastCall("CatalistSwapV3Test#test_v3Swap_exactInput_MultiHop");
        assertEq(ERC20(token0()).balanceOf(FROM), BALANCE - AMOUNT);
    ...
    function test_v3Swap_exactOutput0For1() public {
        ...
        router.execute(commands, inputs);
        snapLastCall("CatalistSwapV3Test#test_v3Swap_exactOutput0For1");
    ...
    function test_v3Swap_exactOutput_MultiHop() public {
        ...
        router.execute(commands, inputs);
        snapLastCall("CatalistSwapV3Test#test_v3Swap_exactOutput_MultiHop");
        assertLt(ERC20(token0()).balanceOf(FROM), BALANCE);
        assertEq(ERC20(token1()).balanceOf(FROM), BALANCE);
    ...
    function test_v3Swap_catalistV3SwapCallback_InvalidCaller() public {
        bytes memory path = abi.encodePacked(token1(), fee(), token0());
        bytes memory data = abi.encode(path, makeAddr("payer"));

        vm.expectRevert(V3SwapRouter.V3InvalidCaller.selector);
        router.catalistV3SwapCallback(100, 100, data);
    }
